...
#RETURN_CODE=$(echo "$INPUT" | cut -d'|' -f7)
#echo "[INFO]: User Input in Find Form GUI: $INPUT"
#echo "Return Code OF FIND GUI: $RETURN_CODE" ; 

#> Valdate Return code of Find GUI, if Cancel exit application
# if [ $RETURN_CODE -eq 1 ]; then
# 	echo "[INFO]: User Exit The  Find GUI Application!"
# 	exit
# fi
...
# user-trash-symbolic
#gtk-delete
function show_list_of_searched_files(){
	if [ $TYPE == "File" ]; then
		yad_cmd='yad --center --undecorated --text="[MKM]: Search Result\n$LINE"  --height=500 --list --button="Delete!user-trash-symbolic:2" --button="View!document-preview:0" --button="Locate!folder-symbolic:4" --button="Close!gtk-close:6" --image="$(pwd)/icons/humming-bird_5.png" --window-icon="$(pwd)/icons/humming-bird_16.png" --column="SIZE" --column="DATE & TIME" --column="File Path"'
	else
		yad_cmd='yad --center --undecorated --text="[MKM]: Search Result\n$LINE"  --height=500 --list --button="Delete!user-trash-symbolic:2" --button="Locate!folder-symbolic:4" --button="Close!gtk-close:6" --image="$(pwd)/icons/humming-bird_5.png" --window-icon="$(pwd)/icons/humming-bird_16.png"  --column="Directory Path"'
	fi
	content=""
	...
	if [ ! -z $SELECTED_ROW ]; then  
		...
		if [ $TYPE == "File" ]; then
			...
			file_path=$(echo $SELECTED_ROW| cut -d'|' -f1)			
		fi
		echo 
		case $RETURN_CODE in
			0)
			...
			4)
				...
				show_list_of_searched_files
				;;
			6) echo "[INFO]: Closing List Dialog!"
				;;
			esac
